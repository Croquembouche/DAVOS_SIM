...
public:
    ...
    bool writeImage(const cv::Mat &image, int &timeStamp);      // Function to write an image to shared memory
...
public:
    ...
    bool readImage(cv::Mat &image, int &last_frame, int &timeStamp); // Function to read image from shared memory
